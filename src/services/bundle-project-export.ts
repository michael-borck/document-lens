...
function rowToDocument(row: DocumentRow): Document {
  return {
    ...
    pdfMetadata: row.pdf_metadata ? (JSON.parse(row.pdf_metadata) as Record<string, unknown>) : null,
    status: row.status,
...
// Suppress unused-type warnings for types only referenced via JSDoc / shape.
export type _Unused = KeywordTag
